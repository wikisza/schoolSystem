...
def messagesTeacher():
    ...
    return render_template('teacher/messagesTeacher.html', username=current_user.username, profession=current_user.profession, rooms=rooms)

@messages_blueprint.route('/messagesStudent')
@login_required
def messagesStudent():
    rooms = get_rooms(current_user.id)
    return render_template('student/messagesStudent.html', username=current_user.username, profession=current_user.profession, rooms=rooms)

@messages_blueprint.route('/messagesParent')
@login_required
def messagesParent():
    rooms = get_rooms(current_user.id)
    return render_template('parent/messagesParent.html', username=current_user.username, profession=current_user.profession, rooms=rooms)
